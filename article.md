...
```
    unleash load on web app - something brakes - stop the load - tune/fix/adjust - load it up again - ...
```
...
First of all, one needs to determine what is the amount of load application is expected to withstand, what kind of requests should be used to generate load and what would be acceptable web app error rate and response time (and possibly other success criteria).


### Scenario and load level

By scenario I mean which web app URLs load testing tool will hit. Here's example load testing scenario:
...
Where one can get these numbers, and which URLs should be in scenario at all? If new web app is replacing existing one (which is frankly speaking true for a majority of new apps), one can mine the logs of the legacy application. It's ok to include in scenario only high-hit pages. E.g. from logs collected over last month, total sum of hits to home, product list and detail pages is 98.3% of all hits, and other page types account for max 0.1% hits per page type. For such case, load testing scenario can include only home, product list and detail page hits. Yet it is important that hits to other page types won't bring the whole web app down. In case "other page types" are all just static pages - probably no need to include them in scenario. If there are page types which are not hit frequently but are heavy on resource usage - might be good idea to include those in main scenario, or run separate smaller-scale load testing session for those, possibly with adjusted success criteria.

Now there's the question of how much load should the tool apply to web app. That is also a question to answer which one can use logs/monitoring of legacy app. It's likely the case that user load for legacy app fluctuates a lot during the course of day or week. Determine the min, avg and max RPS legacy app handled during say last month (same datetime range used in previous analysis of how load should split between page types), and swing from there. I suggest following formula for RPS in load testing session: `max(avgRPS * 4, 3 * maxRPS)`. Simplest formula good enough for most is `3 * maxRPS`. If user traffic is anticipated to grow in future, use anticipated future `maxRPS` instead of one derived from logs.
...
Simulated user request should be close to real life. If app users are live people (not other apps), it makes sense to introduce small delay between requests simulated user makes (e.g. using [JMeter's random timer](https://www.blazemeter.com/blog/comprehensive-guide-using-jmeter-timers)).

It is also worth to take a look at HTTP connection settings used by load testing tool. It might make sense to reuse HTTP connection for requests from same artificial user for some number of requests, to simulate user interacting with web app in web browser better. It might be also worthwhile to set HTTP version and adjust other options like socket timeout (configure JMeter's `LoopController.loops` for thread group, set `HttpClient` to [HttpClient4 class](https://jmeter.apache.org/usermanual/properties_reference.html#httpclient4)).
...
What if app is completely new, there's no logs to mine? Then one might brainstorm together with business folks scenario which best predicts app usage, and possibly use higher value in `N * maxRPS` formulae for load level.
...
Scenario used by load testing tool usually contains definition of "success hit" which can be tweaked to suit your app needs. ([JMeter's ResponseAssertion and DurationAssertion](https://www.blazemeter.com/blog/how-use-jmeter-assertions-three-easy-steps)).


## Load testing tools

There's plenty of load testing tools available - [this post](https://k6.io/blog/comparing-best-open-source-load-testing-tools/) lists some of them.

Long-existing tool used by lots of people is JMeter. It's good fit for those who need involved scenario and prefer GUI interactions over writing code. It's possible to run it with master+worker nodes setup from cloud ([how to do it](https://github.com/kubernauts/jmeter-kubernetes)), or use pay-for online-based platforms which run JMeter for you.

Another tool which gains popularity quickly particularly for cloud-based setup is [k6](https://k6.io/). It's good fit if you don't mind JavaScript and need something cloud-native. GitLab has [example](https://docs.gitlab.com/ee/user/project/merge_requests/load_performance_testing.html) on how to run k6 in a job (possibly for short term load session as part of app rollout). Or one can run k6 in separate from web app cloud project instead.
...
In case you're good with Python and don't need high RPS level, [locust](https://locust.io/) might be good fit. It is rather easy to write scenario for.
...
It makes sense to first load test backends and DBs web app uses - calculate what load is expected for backend, and apply that directly to backend. Load tool probably needs to be inside your infrastructure for such session. Make sure backends and DBs web app uses are handling load alright (satisfy SLA), then switch to load testing web app itself. It might be cheaper this way - load tests are expensive in terms of burned resources.

It's a good idea to run smaller load session for web app, before starting big load: same scenario as for high load with reduced load and shorter duration. E.g., load test one web app pod, make sure it works OK for CPU load level up to scaling threshold value, then proceed to testing under high load.
...
Outages are best tested over main high load scenario, doesn't have to be long-running session.
...
- list of what problems surfaced during the session, investigation and mitigation notes
